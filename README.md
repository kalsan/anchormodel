...
Both scopes accept `String`, `Symbol`, or `Anchormodel` instances (and nested arrays), and raise on invalid keys. They are defined for every `belongs_to_anchormodels` attribute, named `with_any_<attribute_name>` / `with_all_<attribute_name>`.
